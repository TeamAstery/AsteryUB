...
async def alive_handler(_, m: Message):
    try:
        ...
        alive_msg = f"\n"
        if app.UserBio():
            alive_msg += f"⦿ I Am Running Perfect.\n\n"
        alive_msg += f"⟜ **Owner:** {app.UserMention()}\n"
        alive_msg += f"⟜ **Astery** `{app.userbot_version}`\n"
        ...
        if (pic) and (pic.endswith(".mp4" or ".mkv" or ".gif")):
            await app.send_video(
                m.chat.id, pic, caption=alive_msg, parse_mode="markdown"
            )
...
async def inlinealive_handler(_, m: Message):
    ...
    try:
        ...
        await app.send_edit(
            m, "Turning inline mode to on, wait . . .", text_type=["mono"]
        )
    ...
    if result:
        ...
        await app.send_edit(
            m, "Something went wrong, please try again later . . .", delme=2
        )
...
async def inlinequote_handler(_, m: Message):
    try:
        ...
        if result:
            ...
            await app.send_edit(
                m, "Please try again later !", delme=2, text_type=["mono"]
            )
    except Exception as e:
        await app.error(m, e)
